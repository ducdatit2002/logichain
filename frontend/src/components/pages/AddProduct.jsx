...
const AddProduct = () => {
  ...
  const getUsername = async (e) => {
    const res = await axios
      .get(`http://localhost:8080/profile/${auth.user}`)
      .then((res) => {
  ...
  const uploadImage = async (image) => {
    ...
    axios
      .post("http://localhost:8080/upload/product", data, {
  ...
  const checkUnique = async () => {
    const res = await axios.get("http://localhost:8080/product/serialNumber");
